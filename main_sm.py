...
from utils.settings_CRNN import *#_sm import *
...
def main(iteration=None):
    ...
    #set configurations
    configs, server_cfg, train_cfg, feature_cfg = get_configs(config_dir="./configs/config_sm.yaml")
        ...
        for _, (wavs, labels, _, indexes, filenames, paths) in enumerate(tk1, 0):
            ...
            if torch.any(mask_strong):
                ...
                for th in val_stud_buffer.keys():
                    val_stud_buffer[th] = pd.concat([val_stud_buffer[th], pd.DataFrame(stud_pred_dfs[th])])
#                     val_stud_buffer[th].append(stud_pred_dfs[th], ignore_index=True)
                for th in val_tch_buffer.keys():
                    val_tch_buffer[th] = pd.concat([val_tch_buffer[th], pd.DataFrame(tch_pred_dfs[th])])
#             val_tch_buffer[th].append(tch_pred_dfs[th], ignore_index=True)
    ...
    with torch.no_grad():
        ...
        for _, (wavs, labels, _, indexes, filenames, paths) in enumerate(tk2, 0):
            ...
            for th in stud_test_psds_buffer.keys():
                stud_test_psds_buffer[th] = pd.concat([stud_test_psds_buffer[th], pd.DataFrame(stud_pred_dfs[th])])
#                 stud_test_psds_buffer[th].append(stud_pred_dfs[th], ignore_index=True)
            for th in tch_test_psds_buffer.keys():
                tch_test_psds_buffer[th] = pd.concat([tch_test_psds_buffer[th], pd.DataFrame(tch_pred_dfs[th])])
#             tch_test_psds_buffer[th].append(tch_pred_dfs[th], ignore_index=True)
            ...
            tch_pred_df_halfpoint = decode_pred_batch(tch_preds, weak_tch_preds, paths, encoder, [0.5],
                                                      train_cfg["median_window"], train_cfg["decode_weak_test"])
            stud_test_f1_buffer = pd.concat([stud_test_f1_buffer, pd.DataFrame(stud_pred_df_halfpoint[0.5])])
#             stud_test_f1_buffer.append(stud_pred_df_halfpoint[0.5], ignore_index=True)
            tch_test_f1_buffer = pd.concat([tch_test_f1_buffer, pd.DataFrame(tch_pred_df_halfpoint[0.5])])
#     tch_test_f1_buffer.append(tch_pred_df_halfpoint[0.5], ignore_index=True)
